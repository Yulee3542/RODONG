...
class RodongMain:
    ...
    def reverse_motor(self, speed=25, target_front_cm=30, avoid_dir=0):
        """전방 센서 기준 일정 거리까지 후진, 회피 방향 + 후방 센서 기반 조향"""
        self.reversing = True
        ...
        if avoid_dir > 15:
            initial_steer = 90
            rospy.loginfo('[Reverse] 우측 회피 중 → 우조향(90°)')
        elif avoid_dir < -15:
            initial_steer = -90
            rospy.loginfo('[Reverse] 좌측 회피 중 → 좌조향(-90°)')
        else:
            initial_steer = 0
        ...
        while not rospy.is_shutdown() and time.time() < timeout:
            ...
            if 0 < rear_c < SONAR_REVERSE:
                ...
                steer = -90   # 좌후 막힘 → 왼쪽 조향 (바퀴를 장애물쪽)
                rospy.loginfo('[Reverse] 좌후(%.0fcm) → 좌조향(-90°)', rear_l)
            elif 0 < rear_r < SONAR_REVERSE:
                steer = 90    # 우후 막힘 → 오른쪽 조향 (바퀴를 장애물쪽)
                rospy.loginfo('[Reverse] 우후(%.0fcm) → 우조향(90°)', rear_r)
            else:
                steer = 0
    ...
    def get_avoid_steer_from_sonar(self):
        ...
        front_valid = {}
        if 0 < front_l < 200:
            front_valid['left'] = front_l
        if 0 < front_c < 200:
            front_valid['center'] = front_c
        if 0 < front_r < 200:
            front_valid['right'] = front_r
        
        if not front_valid:
            return 0
        
        closest = min(front_valid, key=front_valid.get)
        min_dist = front_valid[closest]
        ...
        if closest == 'left':
            ...
            l_dist = front_valid.get('left', 999)
            r_dist = front_valid.get('right', 999)
            if l_dist < r_dist:
                steer = 90
                rospy.loginfo('[Sonar] 정면(%.0fcm) 좌측 < → 우조향', min_dist)
            else:
                steer = -90
                rospy.loginfo('[Sonar] 정면(%.0fcm) 우측 < → 좌조향', min_dist)
    ...
    def _bug_drive_step(self):
        ...
        if not self.avoiding:
            ...
            if sonar_close or vfh_sharp:
                ...
                self.avoid_t0 = now
                rospy.loginfo('[Avoid] 진입 (steer=%d°)', self.avoid_ang)
        ...
        if self.avoiding:
            ...
            if held >= AVOID_HOLD_MIN and front >= AVOID_CLEAR_CM:
                self.avoiding = False
                rospy.loginfo('[Avoid] 해제 (front=%.0fcm)', front)
        else:
